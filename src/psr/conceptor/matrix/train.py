...
def sweep(
    ...
    """Grid over (layer, alpha, nll_weight). Every row in the resulting sweep JSONL carries
    ...
    def train_fn(point: dict) -> dict:
        ...
        print(f"layer={point['layer']} alpha={point['alpha']} nll_weight={point['nll_weight']} "
              f"final_mse={result['final_mse']:.4f} participation_ratio={result['participation_ratio']:.3f}")
