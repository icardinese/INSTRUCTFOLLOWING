...
LOSS_BALANCE = os.environ.get("PSR_CONCEPTOR_LOSS_BALANCE", DEFAULT_LOSS_BALANCE)
REG_COEFF = float(os.environ.get("PSR_CONCEPTOR_REG_COEFF", loss_balance(LOSS_BALANCE)["reg_coeff"]))
...
MSE_WEIGHT = float(os.environ.get("PSR_CONCEPTOR_MSE_WEIGHT", 1.0))
...
def sweep(
    ...
    def train_fn(point: dict) -> dict:
        ...
        _gate_store.save(_gate_store.point_path(adapter.RESULTS_DIR, "conceptor", point, _fp), "conceptor", result)
        print(f"layer={point['layer']} alpha={point['alpha']} mse_weight={point['mse_weight']} "
              f"nll_weight={point['nll_weight']} final_mse={result['final_mse']:.4f} final_nll={result['final_nll']:.4f}")
    ...
    torch.save({
        ...
        "mse_weight": best["mse_weight"], "nll_weight": best["nll_weight"],
        "task": task, "completed_epochs": winner["completed_epochs"],
    }, out_probe_path)
